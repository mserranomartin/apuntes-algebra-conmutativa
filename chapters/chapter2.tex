...
\begin{definition}
	Sea $M\in\Mod_A$ tal que para toda $g\in\Hom_A(N,N')$ suprayectiva y toda $\varphi\in\Hom_A(M,N'')$ existe $h\in\Hom_A(M,N)$ verificando que completa el diagrama: $g\circ\varphi=h$. En estas condiciones, decimos que $M$ es un \textit{$A$-módulo proyectivo}.
\end{definition}
...
\begin{figure}[h!]
  ...
  % Text Node
  \draw (262.4,72) node [anchor=north west][inner sep=0.75pt]    {$N$};
  % Text Node
  \draw (351.8,72.4) node [anchor=north west][inner sep=0.75pt]    {$N'$};
  % Text Node
  \draw (261.2,143.6) node [anchor=north west][inner sep=0.75pt]    {$M$};
  % Text Node
  \draw (303.72,55.6) node [anchor=north west][inner sep=0.75pt]    {$g$};
  % Text Node
  \draw (323.32,117.6) node [anchor=north west][inner sep=0.75pt]    {$\varphi $};
  % Text Node
  \draw (246.92,106.2) node [anchor=north west][inner sep=0.75pt]    {$h$};
  % Connection
  \draw    (280.4,79.65) -- (348.8,79.94) ;
  \draw [shift={(348.8,79.94)}, rotate = 180.25] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (17.64,-3.29) .. controls (13.66,-1.4) and (10.02,-0.3) .. (6.71,0) .. controls (10.02,0.3) and (13.66,1.4) .. (17.64,3.29)(10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
  % Connection
  \draw    (282.2,141.87) -- (347.22,91.33) ;
  \draw [shift={(348.8,90.1)}, rotate = 502.14] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
  % Connection
  \draw    (270.15,139.2) -- (269.96,93.6) ;
  \draw [shift={(269.95,91.6)}, rotate = 449.76] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
...
\begin{theorem}\label{caracplano}Sea $M$ un $A$-módulo. Las siguientes afirmaciones son equivalentes.
	\begin{itemize}
		\item[1.] $M$ es un $A$-módulo plano.
		...
		\item[3.] Si $$\sum_{i=1}^na_im_i=0_A$$ para ciertos $a_i\in A$ y $m_i\in M$, entonces existen ${m_j}'\in M$ de forma que para cada $i$ se tiene $$m_i=\sum_{j=1}^s\lambda_{ij}{m_j}',\hspace{15pt}\lambda_{ij}\in A$$ y para cada $j$ se verifica $$\sum_{i=1}^n\lambda_{ij}a_i=0.$$
...
\begin{theorem}
	...
	\begin{itemize}
		\item[1)]  $M$ es un $A$-módulo plano.
		\item[2)] Si $$\sum_{i=1}^na_im_i=0_A$$ para ciertos $a_i\in A$ y $m_i\in M$, entonces existen ${m_j}'\in M$ de forma que para cada $i$ se tiene $$m_i=\sum_{j=1}^s\lambda_{ij}{m_j}',\hspace{15pt}\lambda_{ij}\in A$$ y para cada $j$ se verifica $$\sum_{i=1}^n\lambda_{ij}a_i=0.$$
