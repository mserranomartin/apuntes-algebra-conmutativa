...
($\Leftarrow$) Supongamos $\p^{ec}=\p$ y tomemos $S:=A\setminus\p$ como conjunto multiplicativamente cerrado. El conjunto $S':=\phi(S)\subset B$ es multiplicativamente cerrado. Tomando las aplicaciones correspondientes $\delta:=\delta_S$ y $\delta':=\delta_{S'}$ tenemos que la composición $\delta'\circ\phi$ es una aplicación que, por definición de $S'$ manda unidades de $S^{-1}A$ a unidades de $S'^{-1}B$, esto es, elementos de $S$ a elementos de $S'.$ De esta forma, por la propiedad universal del anillo de fracciones existe un homomorfismo $\Phi:S^{-1}A\rightarrow S'^{-1}B$ que cumple $$\delta'\circ\phi=\Phi\circ\delta.$$
...
\begin{theorem}[Going-down]Sea $A\subset B$ una extensión entera donde $A$ y $B$ son dominios de integridad y $A$ es íntegramente cerrado. Sean $\p_0,\p_1\in\Spec A$ y $\q_0\in\Spec B$ tales que $\p_0\supset\p_1$ y $\q_0\cap A=\p_0.$ Bajo estas hipótesis, existe $\q_1\in\Spec B$ tal que $\q_0\supset\q_1$ y $\q_1\cap A=\p_1.$
\end{theorem}
