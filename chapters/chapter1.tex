...
\begin{lema}\label{ext-cont-ext}
	Sea $f:A\to B$ un homomorfismo. Si $\af$ es un ideal de $A$, entonces $\af^e = \af^{ece}$. Si $\bfr$ es un ideal de $B$, entonces $\bfr^{cec} = \bfr^c$.
\end{lema}

\begin{proof}
	Sea $\af$ un ideal de $A$. Usando las propiedades de la observación \ref{prop_ext_con}, $\af \subset \af^{ec}$ y así $\af^e \subset \af^{ece}$. Recíprocamente, $\af^{ece} = (\af^e)^{ce} \sub \af ^e$.
