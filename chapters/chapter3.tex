...
>>>>>>> Stashed changes
\end{remark}
\begin{lemma} Sean $Z$ y $Z'$ dos $A$-módulos. Sea $\{z_i\}_{i\in I}$ un sistema de generadores de $Z$ y sea $\{z_j'\}_{j\in J}$ un sistema de generadores de $Z'$. Entonces, $\{z_i\otimes z_j:(i,j)\in I\times J\}$ es un sistema de generadores de $Z\otimes_AZ'$.
\end{lemma}
...
\end{array}
$$
Es una comprobación rutinaria ver que $F$ es una aplicación trilineal. Por tanto, existe una única $f:M\otimes_AN\otimes_AP\rightarrow (M\otimes_AN)\otimes_AP$ tal que $f(m\otimes n\otimes p)=f(\delta(m,n,p))=F(m,n,p)=(m\otimes n)\otimes p$, donde $\delta:M\otimes N\otimes P\rightarrow M\otimes_AN\otimes_AP$ es la aplicación que se obtiene en la proposición anterior.

Veamos como definir la flecha en sentido contrario. Para cada $p\in P$, definimos
$$\begin{array}{rrcl}
	\Phi_p:&M\times N&\longrightarrow&M\otimes_AN\otimes_AP  \\
	&(m,n)&\longmapsto&m\otimes n\otimes p
	\end{array}$$
Es una comprobación rutinaria ver que es una apliación $A$-bilineal. Por tanto, existe una única $\varphi_p:M\otimes_AN\rightarrow M\otimes_AN\otimes_AP$ tal que $\varphi_p(m\otimes n)=\Phi_p(m,n)=m\otimes n\otimes p$.

Se comprueba también que para cada $p,p'\in P, \lambda\in A$, $\varphi_p\circ\varphi_{p'}=\varphi_{p+p'}$ y $\varphi_{\lambda p}=\lambda\varphi_p$. Entonces, definimos
$$\begin{array}{rrcl}
	G:&(M\otimes_AN)\times P&\longrightarrow&M\otimes_AN\otimes_AP  \\
	&(z,p)&\longmapsto&\varphi_p(z)
	\end{array}$$
Vuelve a ser rutinario ver que $G$ es $A$-bilineal. Entonces, existe una única $g:(M\otimes_AN)\otimes_AP\rightarrow M\otimes_AN\otimes_AP$ $A$-lineal tal que $g((m\otimes n)\otimes p)=G((m\otimes n),p)=\varphi_p(m\otimes n)=m\otimes n\otimes p$.

<<<<<<< Updated upstream
Por un lado, $(g\circ f)(m\otimes n\otimes p)=g((m\otimes n)\otimes p)=m\otimes n\otimes p$. Por tanto, $g\circ f=Id_{M\otimes N\otimes P}$

Por otro, $\{m\otimes n:(m,n)\in M\times N\}$ es sistema de generadores de $M\otimes_AN$. Por el lema anterior, $\{(m\otimes n)\otimes p: (m,n,p)\in M\times N\times P\}$ es sistema de generadores de $(M\otimes_AN)\otimes_AP$. Evaluando, $(f\circ g)((m\otimes n)\otimes p)=(m\otimes n)\otimes p)$ y concluimos $f\circ g)=Id_{(M\otimes_AN)\otimes_AP}$
\end{proof}
=======
