...
\begin{remark}
\begin{enumerate}
  \item A las clases $[e_{(m,n)}]$ se les denota $m\otimes_An$ o simplemente $m\otimes n$.
  ...
  \item Las aplicaciones bilineales de $M\times N$ en $P$, $Bil_A(M\times N,P)$ están en correspondencia biyectiva con $Hom_A(M\otimes_AN,P)$.
...
\begin{definition}
  Al $A$-módulo $M\otimes_AN$ se le llama \emph{producto tensorial} de $M$ y $N$.
\end{definition}

En particular, si tomamos $A$ como $K$ cuerpo y $M$ y $N$ $K$-espacios vectoriales,$$Hom_A(M\otimes_KN,K)=(M\otimes_KN)^{\ast}=Bil_K(M\times N,K)$$

\textit{3)} La construcción del producto tensorial de módulos se puede generalizar. Dados $M_1,\dots,M_r$ $A$-módulos, existe un $A$-módulo $M_1\otimes_A\dots\otimes_AM_r$ y$$\delta:M_1\times\dots\times,M_r\longrightarrow M_1\otimes_A\dots\otimes_AM_r$$ multilineal tal que para cualquier $$\Phi:M_1\times\dots\times,M_r\longrightarrow P$$ $A$-multilineal, existe una única $$f:M_1\otimes_A\dots\otimes_AM_r\longrightarrow P$$ $A$-lineal tal que $f\circ\delta=F$

\end{remark}
\begin{lemma} \label{generadores} Sean $Z$ y $Z'$ dos $A$-módulos. Sea $\{z_i\}_{i\in I}$ un sistema de generadores de $Z$ y sea $\{z_j'\}_{j\in J}$ un sistema de generadores de $Z'$. Entonces, $\{z_i\otimes z_j:(i,j)\in I\times J\}$ es un sistema de generadores de $Z\otimes_AZ'$.
\end{lemma}
\begin{proposition} Sea $A$ un anillo conmutativo unitario. Se cumple:\begin{enumerate}
    \item Dados $M,N$ y $P$ $A$-módulos, $$M\otimes_AN\otimes P\overset{isom}{\cong}(M\otimes_AN)\otimes_AP$$
    \item $M\otimes_AN=N\otimes_AM$
    \item Dados $f:M_1\rightarrow M_2$ y $g:N_1\rightarrow N_2$ $A$-lineales, existe $f\otimes g:M_1\otimes_AN_1\rightarrow M_2\otimes N_2$ $A$-lineal tal que si tenemos $f':M_2\rightarrow M_3$ y $g':N_2\rightarrow N_3$ homomorfismos de $A$-módulos, $$M_1\otimes_AN_1\overset{f\otimes_Ag}{\longrightarrow} M_2\otimes_AN_2\overset{f'\otimes_Ag'}{\longrightarrow} M_3\otimes_AN_3$$ se cumple $$(f'\otimes_Ag')\circ(f\otimes_Ag)=(f'\circ f)\otimes_A(g'\circ g)$$
    \item Si $B$ es un $A$-álgebra, $B\otimes_AM$ es un $B$-módulo
    \item Si $B$ y $C$ son $A$-álgebras, $B\otimes_AC$ es un $A$-álgebra, un $B$-módulo y un $C$-módulo
    \end{enumerate}
\end{proposition}
\begin{proof}
...
$$ \begin{array}{rrcl}
     F:&M\times N\times P&\longrightarrow&(M\otimes_AN)\otimes_AP\\
     &(m,n,p)&\longmapsto&(m\otimes n)\otimes p
\end{array}
$$
Existe una única $f:M\otimes_AN\otimes_AP\rightarrow (M\otimes_AN)\otimes_AP$ tal que $f(m\otimes n\otimes p)=F(m,n,p)=(m\otimes n)\otimes p$,
...
$$\begin{array}{rrcl}
	\Phi_p:&M\times N&\longrightarrow&M\otimes_AN\otimes_AP  \\
	&(m,n)&\longmapsto&m\otimes n\otimes p
	\end{array}$$
Existe una única $\varphi_p:M\otimes_AN\rightarrow M\otimes_AN\otimes_AP$ tal que $\varphi_p(m\otimes n)=\Phi_p(m,n)=m\otimes n\otimes p$
...
$$\begin{array}{rrcl}
	G:&(M\otimes_AN)\times P&\longrightarrow&M\otimes_AN\otimes_AP  \\
	&(z,p)&\longmapsto&\varphi_p(z)
	\end{array}$$
Existe una única $g:(M\otimes_AN)\otimes_AP\rightarrow M\otimes_AN\otimes_AP$ aplicación $A$-lineal que hace conmutativo el diagrama siguiente
...
\begin{figure}
  ...
  % Text Node
  \draw (211.2,82.9) node [anchor=north west][inner sep=0.75pt]    {$( M\otimes N) \times P$};
  % Text Node
  \draw (373.7,82.9) node [anchor=north west][inner sep=0.75pt]    {$M\otimes N\otimes P$};
  % Text Node
  \draw (323.7,67.4) node [anchor=north west][inner sep=0.75pt]    {$g$};
  % Text Node
  \draw (59.2,83.9) node [anchor=north west][inner sep=0.75pt]    {$M\otimes N\otimes P$};
  % Text Node
  \draw (160.2,69.9) node [anchor=north west][inner sep=0.75pt]    {$f$};
  % Text Node
  \draw (62.2,122.9) node [anchor=north west][inner sep=0.75pt]    {$m\otimes n\otimes p$};
  % Text Node
  \draw (216.7,123.4) node [anchor=north west][inner sep=0.75pt]    {$( m\otimes n) \otimes p$};
  % Text Node
  \draw (376.7,122.9) node [anchor=north west][inner sep=0.75pt]    {$m\otimes n\otimes p$};
  % Connection
  \draw    (303.2,90.5) -- (368.7,90.5) ;
  \draw [shift={(370.7,90.5)}, rotate = 180] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
  % Connection
  \draw    (140.2,91.23) -- (206.2,90.81) ;
  \draw [shift={(208.2,90.8)}, rotate = 539.64] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
  % Connection
  \draw    (136.2,130.62) -- (211.7,130.86) ;
  \draw [shift={(213.7,130.87)}, rotate = 180.18] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
  \draw [shift={(136.2,130.62)}, rotate = 180.18] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (0,5.59) -- (0,-5.59)   ;
  % Connection
  \draw    (301.7,130.85) -- (371.7,130.63) ;
  \draw [shift={(373.7,130.62)}, rotate = 539.81] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (10.93,-3.29) .. controls (6.95,-1.4) and (3.31,-0.3) .. (0,0) .. controls (3.31,0.3) and (6.95,1.4) .. (10.93,3.29)   ;
  \draw [shift={(301.7,130.85)}, rotate = 539.81] [color={rgb, 255:red, 0; green, 0; blue, 0 }  ][line width=0.75]    (0,5.59) -- (0,-5.59)   ;
...
Por otro, $\{m\otimes n:(m,n)\in M\times N\}$ es sistema de generadores de $M\otimes_AN$. Por el lema \ref{generadores}, $\{(m\otimes n)\otimes p: (m,n,p)\in M\times N\times P\}$ es sistema de generadores de $(M\otimes_AN)\otimes_AP$. Evaluando, $(f\circ g)((m\otimes n)\otimes p)=(m\otimes n)\otimes p)$ y concluimos $f\circ g)=Id_{(M\otimes_AN)\otimes_AP}$
...
\begin{itemize}
    \item [1)] Dados $M_1,M_2$ y $M_3$ $A$-módulos, $M_1\otimes_A M_2\otimes M_3\overset{isom}{\cong}(M_1\otimes_AM_2)\otimes_AM_3\overset{isom}{\cong}M_1\otimes_A(M_2\otimes_AM_3)$
    \item [2)] $M\otimes_AN=N\otimes_AM$
    \item [3)] Dados $f:M_1'\to M_1$ y $g:M_2'\to M_2$ $A$-lineales, existe $f\otimes g:M_1'\otimes_A M_2'\rightarrow M_1\otimes M_2$ $A$-lineal tal que el diagrama es conmutativo.
