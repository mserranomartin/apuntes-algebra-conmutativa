...
\begin{definition} Una sucesión de homomorfismos de $A$-módulos$$...\longrightarrow M_{i-1}\longrightarrow M_i\longrightarrow M_{i+1}\longrightarrow...$$ se dice exacta si $ker(\Phi_{i+1})=im(\Phi_i)$, donde para cada $i$, $M_i$ es un $A$-módulo y $\Phi_i:M_i\rightarrow M_{i+1}$ es un homomorfismo de $A$-módulos.
\end{definition}
\begin{definition} Decimos que una sucesión de homomorfismos de $A$-módulos es corta si es de la forma$$0\longrightarrow M_1\longrightarrow M_2\longrightarrow M_3\longrightarrow 0$$
\end{definition}
...
\end{remark}
\begin{remark} Una sucesión corta estándar viene dada por $N\subset M$, donde $N$ y $M$ son $A$-módulos, con$$0\longrightarrow N\longrightarrow M\longrightarrow\faktor{M}{N}\longrightarrow 0$$
\end{remark}
