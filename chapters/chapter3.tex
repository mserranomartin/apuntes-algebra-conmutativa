...
\textit{2)} Dados $M$ y $N$ $A$-módulos,$$0\longrightarrow M\overset{q_M}{\longrightarrow} M\bigoplus N\overset{p_N}{\longrightarrow} N\longrightarrow 0$$ es una también una sucesión corta exacta.
\end{example}
...
\end{definition}
\begin{remark} Dado $K$ cuerpo, los $K$-módulos son los $K$-espacios vectoriales. Tomando $\zeta$ como los $K$-espacios vectoriales de dimensión finita,
$$\begin{array}{rcl}
...
\end{remark}
\begin{proposition} Sea$$0\longrightarrow M'\overset{f}{\longrightarrow} M\overset{g}{\longrightarrow} M''\longrightarrow 0$$ una sucesión corta y exacta de $A$-módulos. Son equivalentes:
	\begin{itemize}
		\item [i)] Existe $\tau:M\longrightarrow M'$ homomorfismo de $A$-módulos tal que $\tau\circ f=\text{id}_{M'}$
		\item[ii)] Existe $\sigma:M''\longrightarrow M$ homomorfismo de $A$-módulos tal que $g\circ\sigma=\text{id}_{M''}$
		\item[iii)] $M\cong M'\bigoplus M''$ vía $f$ y $g$, es decir, existe $\Phi:M\longrightarrow M'\bigoplus M''$ isomorfismo de $A$-módulos tal que los diagramas son conmutativos.
	\end{itemize}
	En tal caso, se dice que la sucesión corta es \textit{escindida}.
\end{proposition}
Demostración. $(1\Rightarrow 2)$ Dado $m''\in M''$, por ser $g$ sobreyectiva existe $m\in M$ tal que $g(m)=m''$. Considero $$m^*:=m-f\circ\tau(m)\in M$$
...
$$g\cir\sigma(m'')=g(\sigma(m''))=g(m-f\circ\tau(m))=g(m)=m'',$$
