...
\begin{document}
	...
	\begin{remark}
		...
		2) De la Propiedad universal del anillo de fracciones se deduce que, en el caso de que $A$ sea un DI y $S=A \setminus \operatorname{Div_0}(A) = A \setminus \{0\}$, $S^{-1}A$ es el menor cuerpo que contiene a $A$, el cuerpo de fracciones de $A$.
...
\begin{theorem}\textnormal{\textbf{(de la raíz racional)}}\label{rational_root}
	Sea $A$ un DFU, $K_A$ su cuerpo de fracciones, y $f = \sum_{i=0}^n a_i X^i \in A[X]$ un polinomio. Si $\alpha=\frac{p}{q}\in K_A$ es una raíz de $f$ con $p$ y $q$ coprimos, entonces $a_0 \in \langle p \rangle $ y $a_n \in \langle q \rangle$.
\end{theorem}
\begin{proof}
Si $f(\alpha) = 0$, multiplicando por $q^n$ y despejando el término en $a_0$ obtenemos $$ a_0q^n = a_np^n + a_{n-1}p^{n-1} + \dots a_1pq^{n-1} = p(a_np^{n-1} + a_{n-1}p^{n-2} + \dots a_1q^{n-1}) $$
y como $p$ y $q$ son coprimos, entonces $p$ divide a $a_0$. Razonando análogamente se obtiene que $q$ divide a $a_n$.
\end{proof}
